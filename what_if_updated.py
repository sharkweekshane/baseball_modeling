...
ted_stats = load_and_clean_data('ted_stats.csv', 
                                ['Season', 'Name', 'Age', 'R', 'HR', 'RBI', 'SB', 'AVG', 'OBP', 'wRC+', 'WAR'], 
                                ['Season', 'Name', 'Age', 'R', 'HR', 'RBI', 'SB', 'AVG', 'OBP', 'wRC+', 'WAR'])

joe_stats = load_and_clean_data('joe_stats.csv', 
                                ['Season', 'Name', 'Age', 'R', 'HR', 'RBI', 'SB', 'AVG', 'OBP', 'wRC+', 'WAR'], 
                                ['Season', 'Name', 'Age', 'R', 'HR', 'RBI', 'SB', 'AVG', 'OBP', 'wRC+', 'WAR'])

# Create GT tables with titles
ted_gt_table = GT(ted_stats).tab_header(title="Ted Williams' Stats")
joe_gt_table = GT(joe_stats).tab_header(title="Joe DiMaggio's Stats")

# Save the tables as PNGs
ted_gt_table.save("ted_stats_1.png")
joe_gt_table.save("joe_stats_1.png")
...
def display_leaderboard_changes(player, stat):
    ...
    return pre_leaderboard, post_leaderboard
...
# Function to create and save GT tables
def create_and_save_tables(player, stat, pre_leaderboard, post_leaderboard):
    pre_title = f"{player}'s Pre-Update {stat} Leaderboard"
    post_title = f"{player}'s Post-Update {stat} Leaderboard"
    
    pre_gt_table = GT(pre_leaderboard).tab_header(title=pre_title)
    post_gt_table = GT(post_leaderboard).tab_header(title=post_title)
    
    pre_filename = f"{player.lower().replace(' ', '_')}_pre_{stat.lower()}_leaderboard.png"
    post_filename = f"{player.lower().replace(' ', '_')}_post_{stat.lower()}_leaderboard.png"
    
    pre_gt_table.save(pre_filename)
    post_gt_table.save(post_filename)

# Display and get pre and post leaderboard data for each category
for player in ['Ted Williams', 'Joe DiMaggio']:
    for stat in ['R', 'HR', 'RBI', 'WAR']:
        pre_leaderboard, post_leaderboard = display_leaderboard_changes(player, stat)
        create_and_save_tables(player, stat, pre_leaderboard, post_leaderboard)
...
# Create a summary DataFrame for pre and post ranks
summary_data = []
for stat, ranks in pre_post_ranks.items():
    for player, rank_changes in ranks.items():
        summary_data.append([player, stat, rank_changes['pre'], rank_changes['post']])

summary_df = pd.DataFrame(summary_data, columns=['Player', 'Statistic', 'Pre-Update Rank', 'Post-Update Rank'])

# Create and save the GT table
summary_gt_table = GT(summary_df).tab_header(title="Pre-Update and Post-Update Ranks for Ted Williams and Joe DiMaggio")
summary_gt_table.save("pre_post_ranks_summary.png")
...
# Define the regions and their country codes
oceania = {
    "Australasia": ["AU", "NZ"],
...
# Create the table using great_tables
gt_table = (
...
gt_table.save(file="oceania_populations.png")
...
# Sample data for Joe and Ted's real and simulated stats
joe_stats_2 = pd.DataFrame({
    'Category': ['Runs', 'Home Runs', 'RBIs', 'WAR'],
    'Real': [joe_reg_r, joe_reg_hr, joe_reg_rbi, joe_reg_war],
    'Simulated': [joe_sim_r, joe_sim_hr, joe_sim_rbi, joe_sim_war],
    'Combined': [joe_reg_r + joe_sim_r, joe_reg_hr + joe_sim_hr, joe_reg_rbi + joe_sim_rbi, round(joe_reg_war + joe_sim_war, 3)]
})

ted_stats_2 = pd.DataFrame({
    'Category': ['Runs', 'Home Runs', 'RBIs', 'WAR'],
    'Real': [ted_reg_r, ted_reg_hr, ted_reg_rbi, ted_reg_war],
    'Simulated': [ted_sim_r, ted_sim_hr, ted_sim_rbi, ted_sim_war],
    'Combined': [ted_reg_r + ted_sim_r, ted_reg_hr + ted_sim_hr, ted_reg_rbi + ted_sim_rbi, round(ted_reg_war + ted_sim_war, 3)]
})

# Function to convert integers except for WAR
def convert_to_int_except_war(df):
    for col in df.columns:
        if col != 'Category' and df.loc[df['Category'] != 'WAR', col].dtype == 'float':
            df.loc[df['Category'] != 'WAR', col] = df.loc[df['Category'] != 'WAR', col].astype(int)
    return df

joe_stats_2 = convert_to_int_except_war(joe_stats_2)
ted_stats_2 = convert_to_int_except_war(ted_stats_2)

# Ensure WAR column is formatted with 1 decimal place
joe_stats_2.loc[joe_stats_2['Category'] == 'WAR', ['Real', 'Simulated', 'Combined']] = joe_stats_2.loc[joe_stats_2['Category'] == 'WAR', ['Real', 'Simulated', 'Combined']].applymap(lambda x: f"{x:.1f}")
ted_stats_2.loc[ted_stats_2['Category'] == 'WAR', ['Real', 'Simulated', 'Combined']] = ted_stats_2.loc[ted_stats_2['Category'] == 'WAR', ['Real', 'Simulated', 'Combined']].applymap(lambda x: f"{x:.1f}")

# Create GT tables with titles
joe_gt_table = GT(joe_stats_2).tab_header(title="Joe DiMaggio's Real and Simulated Stats")
ted_gt_table = GT(ted_stats_2).tab_header(title="Ted Williams' Real and Simulated Stats")

# Save the tables as PNGs
joe_gt_table.save("joe_stats.png")
ted_gt_table.save("ted_stats.png")
